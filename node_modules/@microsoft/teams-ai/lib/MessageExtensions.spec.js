...
describe('MessageExtensions', () => {
    ...
    it('should exist when Application is instantiated', () => {
        assert_1.strict.notEqual(mockApp.messageExtensions, undefined);
        assert_1.strict.equal(mockApp.messageExtensions instanceof MessageExtensions_1.MessageExtensions, true);
    });
    describe(`${ANONYMOUS_QUERY_LINK_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with an unfurled link in the response', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(ANONYMOUS_QUERY_LINK_INVOKE_NAME, {
            ...
            await adapter.processActivity(activity, async (context) => {
                ...
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.deepEqual(response.value, {
                    status: 200,
                    body: {
                        composeExtension: {
                            type: 'result',
    ...
    describe(`${CONFIGURE_SETTINGS}`, () => {
        it('should return InvokeResponse with status code 200 with the configure setting invoke name', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(CONFIGURE_SETTINGS, { theme: 'dark' });
            activity.channelId = 'msteams';
            mockApp.messageExtensions.configureSettings(async (context, _state, value) => {
                assert_1.strict.equal(value.theme, 'dark');
            });
            await adapter.processActivity(activity, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.status, 200);
                assert_1.strict.equal(response.value.body, undefined);
            });
        });
    });
    describe(`${FETCH_TASK_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with the task invoke card', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, { commandId: 'showTaskModule' });
            activity.channelId = 'msteams';
            mockApp.messageExtensions.fetchTask('showTaskModule', async (context, _state) => {
                return {
            ...
            await adapter.processActivity(activity, async (context) => {
                ...
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.status, 200);
                assert_1.strict.deepEqual(response.value.body, {
                    task: {
                        type: 'continue',
                        value: {
        ...
        it('should return InvokeResponse with status code 200 with a string message', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, { commandId: 'showMessage' });
            activity.channelId = 'msteams';
            mockApp.messageExtensions.fetchTask('showMessage', async (context, _state) => {
        ...
        it('should call the same handler among an array of commandIds', async () => {
            ...
            const activity = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, {
                commandId: 'showTaskModule'
            });
            activity.channelId = 'msteams';
            const regexp = new RegExp(/show$/, 'i');
            const activity2 = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, {
                commandId: 'Show'
            });
            activity2.channelId = 'msteams';
            const activity3 = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, {
                commandId: 'show task module'
            });
            activity3.channelId = 'msteams';
            const activity4 = (0, TestUtilities_1.createTestInvoke)(FETCH_TASK_INVOKE_NAME, {
                commandId: 'show task'
            });
            ...
            await adapter.processActivity(activity, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.body.task.value.commandId, 'showTaskModule');
            });
            await adapter.processActivity(activity2, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.match(response.value.body.task.value.commandId, regexp);
            });
            await adapter.processActivity(activity3, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.body.task.value.commandId, 'show task module');
            });
            await adapter.processActivity(activity4, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.body.task.value.commandId, 'show task');
            });
        });
        it('should throw an error when the routeSelector routes incorrectly', async () => {
            // Incorrect invoke
            const activity = (0, TestUtilities_1.createTestInvoke)(SUBMIT_ACTION_INVOKE_NAME, {
                commandId: 'Create',
                botActivityPreview: [1],
                botMessagePreviewAction: 'edit'
            });
            mockApp.messageExtensions.fetchTask(async (context) => {
                return context.activity.name === SUBMIT_ACTION_INVOKE_NAME;
            }, async (context, _state) => {
                assert_1.strict.fail('should not have reached this point');
            });
            await adapter.processActivity(activity, async (context) => {
                assert_1.strict.rejects(() => mockApp.run(context), new Error('Unexpected MessageExtensions.fetchTask() triggered for activity type: invoke'));
            });
        });
    });
    describe(`${SUBMIT_ACTION_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with the submit action invoke name for submitAction', async () => {
            ...
            activity.channelId = 'msteams';
            mockApp.messageExtensions.submitAction('giveKudos', async (context, _state, value) => {
                return {
                    type: 'result',
                    attachmentLayout: 'list',
                    attachments: [
        ...
        it('should return InvokeResponse with status code 200 with the submit action invoke name for botMessagePreviewSend', async () => {
            ...
            activity.channelId = 'msteams';
            mockApp.messageExtensions.botMessagePreviewSend('Create Preview', async (context, _state, previewActivity) => {
                assert_1.strict.equal(previewActivity, activity.value.botActivityPreview[0]);
            });
            await adapter.processActivity(activity, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
        ...
        it('should return InvokeResponse with status code 200 with the submit action invoke name for botMessagePreviewEdit', async () => {
            ...
            activity.channelId = 'msteams';
            mockApp.messageExtensions.botMessagePreviewEdit('Create Preview', async (context, _state, previewActivity) => {
                assert_1.strict.equal(previewActivity, activity.value.botActivityPreview[0]);
                return 'edit';
            });
            await adapter.processActivity(activity, async (context) => {
                ...
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.status, 200);
                assert_1.strict.deepEqual(response.value.body.task.value, 'edit');
            });
        });
        it('should call the same handler among an array of commandIds for botMessagePreviewSend', async () => {
            ...
            activity.channelId = 'msteams';
            const activity2 = (0, TestUtilities_1.createTestInvoke)(SUBMIT_ACTION_INVOKE_NAME, {
                commandId: 'preview',
                botActivityPreview: ['preview'],
                botMessagePreviewAction: 'send'
            });
            activity2.channelId = 'msteams';
            mockApp.messageExtensions.botMessagePreviewSend(['create preview', 'preview'], async (context, _state, previewActivity) => {
                assert_1.strict.equal(context.activity.value.commandId, previewActivity);
            });
            await adapter.processActivity(activity, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.notEqual(response, undefined);
            });
            await adapter.processActivity(activity2, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
        ...
        it('should throw an error when the routeSelector routes incorrectly for botMessagePreviewSend', async () => {
            ...
            activity.channelId = 'msteams';
            mockApp.messageExtensions.botMessagePreviewSend(async (context) => {
                return context.activity.name === FETCH_TASK_INVOKE_NAME;
            }, async (context, _state, previewActivity) => {
                assert_1.strict.fail('should not have reached this point');
            });
            await adapter.processActivity(activity, async (context) => {
                assert_1.strict.rejects(() => mockApp.run(context), new Error('Unexpected MessageExtensions.botMessagePreviewSend() triggered for activity type: invoke'));
            });
        });
    });
    describe(`${QUERY_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with the query invoke name', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(QUERY_INVOKE_NAME, { commandId: 'showQuery' });
            activity.channelId = 'msteams';
            mockApp.messageExtensions.query('showQuery', async (context, _state, query) => {
                return {
                    commandId: 'showQuery',
                    ...query
                };
    ...
    describe(`${QUERY_CARD_BUTTON_CLICKED}`, () => {
        it('should return InvokeResponse with status code 200 with the query card button clicked invoke name', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(QUERY_CARD_BUTTON_CLICKED, {
                title: 'Query button',
                displayText: 'Yes',
                value: 'Yes'
            });
            activity.channelId = 'msteams';
            mockApp.messageExtensions.handleOnButtonClicked(async (context, _state, data) => {
                assert_1.strict.equal(data.title, 'Query button');
                assert_1.strict.equal(data.displayText, 'Yes');
                assert_1.strict.equal(data.value, 'Yes');
            });
            await adapter.processActivity(activity, async (context) => {
                await mockApp.run(context);
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.status, 200);
                assert_1.strict.deepEqual(response.value.body, undefined);
            });
        });
    });
    describe(`${QUERY_LINK_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with an unfurled link in the response', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(QUERY_LINK_INVOKE_NAME, {
                url: 'https://www.youtube.com/watch?v=971YIvosuUk&ab_channel=MicrosoftDeveloper'
            });
            activity.channelId = 'msteams';
    ...
    describe(`${QUERY_SETTING_URL}`, async () => {
        it('should return InvokeResponse with status code 200 when querySettingUrl is invoked', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(QUERY_SETTING_URL, {});
            activity.channelId = 'msteams';
            mockApp.messageExtensions.queryUrlSetting(async (context, _state) => {
                return {
            ...
            await adapter.processActivity(activity, async (context) => {
                ...
                const response = context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY);
                assert_1.strict.equal(response.value.status, 200);
                assert_1.strict.deepEqual(response.value.body.composeExtension.value, 'https://fake-url');
            });
        });
    });
    describe(`${SELECT_ITEM_INVOKE_NAME}`, () => {
        it('should return InvokeResponse with status code 200 with selected item in the response', async () => {
            const activity = (0, TestUtilities_1.createTestInvoke)(SELECT_ITEM_INVOKE_NAME, {
                attachmentLayout: 'list',
                attachments: [
                    {
                        contentType: 'application/vnd.microsoft.card.thumbnail',
...
//# sourceMappingURL=MessageExtensions.spec.js.map