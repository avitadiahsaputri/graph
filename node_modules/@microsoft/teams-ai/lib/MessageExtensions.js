...
class MessageExtensions {
    ...
    anonymousQueryLink(handler) {
        const selector = (context) => Promise.resolve(context?.activity?.channelId === 'msteams' &&
            context?.activity?.type == botbuilder_1.ActivityTypes.Invoke &&
    ...
    async returnSubmitActionResponse(context, result) {
        if (context?.activity?.channelId === 'msteams' && !context.turnState.get(botbuilder_1.INVOKE_RESPONSE_KEY)) {
            // Format invoke response
            let response;
            if (typeof result == 'string') {
                // Return message
